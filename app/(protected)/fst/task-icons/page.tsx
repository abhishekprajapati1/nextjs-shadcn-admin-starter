...
const TaskIconsPage = () => {
  const { data, isLoading } = useFetch<any>({
    endpoint: ENDPOINTS.FST.TASK_ICONS.GET_ALL,
  });
              ...
              data.map((icon, index) => {
                return (
                  <React.Fragment key={icon.id}>
                    <tr className="h-30">
                      <td className="bg-white rounded-tl-xl rounded-bl-xl text-start px-4">
                        #{icon.id?.slice(20)}
                      </td>
                      ...
                      <td className="bg-white text-start px-4">
                        {icon._count.tasks} Tasks
                      </td>
                      <td className="bg-white text-start px-4">
                        {bytesToMB(icon.size)} MB
                      </td>
                      <td className="bg-white text-start px-4">
                        {dayjs(icon.upload_date).format("MMM DD, YYYY")}
                      </td>
