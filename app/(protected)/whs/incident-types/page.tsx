...
import useCreateIncidentType from "@/lib/mutations/whs/useCreateIncidentType";
import useDeleteIncidentType from "@/lib/mutations/whs/useDeleteIncidentType";
import useUpdateIncidentType from "@/lib/mutations/whs/useUpdateIncidentType";
...
const IncidentTypesPage = () => {
  const { data, isLoading } = useFetch<IFormOption[]>({
    endpoint: ENDPOINTS.WHS.INCIDENT_TYPES,
  });
  ...
  const { mutate: createType, isPending: creating } = useCreateIncidentType({
    onSuccess: () => {
  ...
  const { mutate: updateType, isPending: updating } = useUpdateIncidentType({
    id: dataToEdit?.id || "",
    onSuccess: () => {
      dispatch(setDataToEdit(null));
      dispatch(setShowForm(false));
