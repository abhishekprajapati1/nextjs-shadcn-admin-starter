...
import useCreateDamageType from "@/lib/mutations/whs/useCreateDamageType";
import useDeleteDamageType from "@/lib/mutations/whs/useDeleteDamageType";
import useUpdateDamageType from "@/lib/mutations/whs/useUpdateDamageType";
...
const DamageTypesPage = () => {
  const { data, isLoading } = useFetch<IFormOption[]>({
    endpoint: ENDPOINTS.WHS.DAMAGE_TYPES,
  });
  const dispatch = useAppDispatch();
  const { showForm, dataToEdit } = useAppSelector((store) => store.typeStore);
  const { mutate: createType, isPending: creating } = useCreateDamageType({
    onSuccess: () => {
  ...
  const { mutate: updateType, isPending: updating } = useUpdateDamageType({
    id: dataToEdit?.id || "",
    onSuccess: () => {
      dispatch(setDataToEdit(null));
      dispatch(setShowForm(false));
          ...
          data.map((data) => {
            return (
              <OptionCard
                key={data.id}
                label={data.label}
                totalLinkedRecords={data._count.damage_types}
                id={data.id}
                useDelete={useDeleteDamageType}
