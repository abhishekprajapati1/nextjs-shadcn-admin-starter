...
const useItems = (configs?: UseItems) => {
    ...
    const { sort_by, total, search_term } = useAppSelector(
        (store) => store.couponManagerStore.dataStore,
    );

    const page_size = completeFetch ? 1000 : 10; // Number of records per page.

    const result = useInfiniteQuery<ICouponManager[]>({
        initialPageParam: 1,
        queryKey: ["coupon-manager", search_term, sort_by],
        queryFn: async ({ pageParam }) => {
            ...
            const response = await api.get(
                ENDPOINTS.admin.coupon_manager.fetch_all(queryString),
            );
