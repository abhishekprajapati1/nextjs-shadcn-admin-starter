...
import { setTotal } from "@/store/frame-materials/data.slice";
...
const useFrameMaterials = (configs?: UseFrameMaterials) => {
  ...
  const result = useInfiniteQuery<IFrameMaterial[]>({
    initialPageParam: 1,
    queryKey: ["frame-materials", search_term, sort_by],
...
export default useFrameMaterials;
