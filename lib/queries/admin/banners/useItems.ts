...
import { setTotal } from "@/store/banners/data.slice";
...
const useItems = (configs?: UseItems) => {
  ...
  const { sort_by, total, search_term } = useAppSelector(
    (store) => store.bannerStore.dataStore,
  );

  const page_size = completeFetch ? 1000 : 10; // Number of records per page.

  const result = useInfiniteQuery<IBanner[]>({
    initialPageParam: 1,
    queryKey: ["banners", search_term, sort_by],
    queryFn: async ({ pageParam }) => {
      ...
      const response = await api.get(
        ENDPOINTS.admin.banners.fetch_all(queryString),
      );
