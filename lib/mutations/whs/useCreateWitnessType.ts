...
type UseCreateWitnessType = {
    onSuccess?: () => void;
}

const useCreateWitnessType = ({ onSuccess }: UseCreateWitnessType) => {
    const queryClient = useQueryClient();
    const mutation = useMutation({
        mutationFn: async (data: IFormOption) => {
            const res = await api.post(ENDPOINTS.WHS.WITNESS_TYPES, data);
            return res.data;
        },
        onSuccess: () => {
...
export default useCreateWitnessType;
