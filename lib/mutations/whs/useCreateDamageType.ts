...
type UseCreateDamageType = {
    onSuccess?: () => void;
}

const useCreateDamageType = ({ onSuccess }: UseCreateDamageType) => {
    const queryClient = useQueryClient();
    const mutation = useMutation({
        mutationFn: async (data: IFormOption) => {
            const res = await api.post(ENDPOINTS.WHS.DAMAGE_TYPES, data);
            return res.data;
        },
        onSuccess: () => {
...
export default useCreateDamageType;
