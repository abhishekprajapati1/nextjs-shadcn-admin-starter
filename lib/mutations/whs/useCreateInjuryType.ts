...
type UseCreateInjuryType = {
    onSuccess?: () => void;
}

const useCreateInjuryType = ({ onSuccess }: UseCreateInjuryType) => {
    const queryClient = useQueryClient();
    const mutation = useMutation({
        mutationFn: async (data: IFormOption) => {
            const res = await api.post(ENDPOINTS.WHS.INJURY_TYPES, data);
            return res.data;
        },
        onSuccess: () => {
...
export default useCreateInjuryType;
